...
// t = i/4). Between them the mapping is a clamped cubic spline: C2, so
// both the camera's speed AND its acceleration are continuous through every
// waypoint. (An earlier per-segment Hermite map was only C1 — the camera
// braked hard onto the B2 close-up, crawled at a constant speed through the
// whole crossfade, then abruptly started accelerating again at the room
// reveal.) The spline enters at the first section's average speed and eases
// to a stop arriving at TOP_VIEW (slope 0 there), since it rests there next.
// Its speed dips smoothly to a near-stop in the middle of the B2 -> Factory
// Interior 1 crossfade, so the dissolve plays on an almost still camera.
// Every waypoint is still reached at exactly the same scroll position.
// (Checked monotone for both knot sets used below.)
const KNOT_XS = [0, EXTERIOR_END, FADE_END, MOVEMENT_END];

function clampedSpline(xs, ys, startSlope, endSlope) {
  const n = xs.length - 1;
  const h = [];
  for (let i = 0; i < n; i++) h.push(xs[i + 1] - xs[i]);
  const alpha = new Array(n + 1);
  alpha[0] = (3 * (ys[1] - ys[0])) / h[0] - 3 * startSlope;
  alpha[n] = 3 * endSlope - (3 * (ys[n] - ys[n - 1])) / h[n - 1];
  for (let i = 1; i < n; i++) {
  ...
  return (x) => {
    ...
    let j = 0;
    while (j < n - 1 && x > xs[j + 1]) j++;
    const t = x - xs[j];
    return ys[j] + b[j] * t + c[j] * t * t + d[j] * t * t * t;
  };
}

function knotSpline(ys) {
  return clampedSpline(KNOT_XS, ys, (ys[1] - ys[0]) / (KNOT_XS[1] - KNOT_XS[0]), 0);
}

const CURVE_T_SPLINE = knotSpline([0, 0.25, 0.5, 0.75]);

function progressToCurveT(progress) {
  const p = Math.min(1, Math.max(0, progress));
  return CURVE_T_SPLINE(p);
}
...
const ARC_SPLINE = knotSpline(POSITION_ARC_AT_WAYPOINT);
const FOV_SPLINE = knotSpline([START.fov, B2_CLOSE.fov, INTERIOR_REVEAL.fov, TOP_VIEW.fov]);

// The look-at point gets the same treatment: sampled by arc length along
// its curve (a Catmull-Rom's own parameter changes speed at every
// waypoint, which read as a small jolt in where the camera points), on a
// C2 spline through its waypoints' arc positions.
LOOKAT_CURVE.arcLengthDivisions = 4000;
const LOOKAT_ARC_SPLINE = knotSpline(
  [0, 1, 2, 3].map((k) => {
    const lengths = LOOKAT_CURVE.getLengths(2000);
    return lengths[Math.round((k / 4) * 2000)] / lengths[2000];
  })
);
...
export function getCameraState(progress) {
  ...
  const lookAt = LOOKAT_CURVE.getPointAt(LOOKAT_ARC_SPLINE(p));

  // fov on the same kind of C2 spline through the waypoints' values, so
  // the zoom has no rate kinks at the waypoints either; `up` only changes
  // on the lift into the top view, so a per-segment lerp is fine for it.
  const fov = FOV_SPLINE(p);
  let up;
  if (p <= EXTERIOR_END) {
    up = lerpUp(START.up, B2_CLOSE.up, p / EXTERIOR_END);
  } else if (p <= FADE_END) {
    up = lerpUp(B2_CLOSE.up, INTERIOR_REVEAL.up, (p - EXTERIOR_END) / (FADE_END - EXTERIOR_END));
  } else if (p <= MOVEMENT_END) {
    // Gradually lifting toward TOP_VIEW while the vehicles move.
    up = lerpUp(INTERIOR_REVEAL.up, TOP_VIEW.up, (p - FADE_END) / (MOVEMENT_END - FADE_END));
  } else {
